...
|---|---|---|---|
| MCU node indoor (kiêm master + IR) | **QR Box Advance Touch Screen** (ESP32-WROOM-32E-N8 + màn 2.8" YT280S030/ST7789V) | ESP32 DevKit V1 trần, ESP32-S3, ESP8266, Raspberry Pi Pico W, STM32 + module WiFi rời | Node này cùng lúc: relay ESP-NOW, IR blaster + IR learner, giữ kết nối MQTT với bộ đệm 12KB (đủ chứa `ir_raw` vài KB), **và hiển thị/điều khiển tại chỗ** — cần nhiều RAM/flash hơn hẳn ESP8266. **Đã chuyển ESP32-S3 → ESP32 DevKit V1 → bo QR Box Advance**: bo này là hàng có sẵn trong kho, gộp luôn màn cảm ứng + RTC + nguồn 9-24VDC công nghiệp vào một bo, thay vì phải tự ghép DevKit + màn rời. Đánh đổi đã biết: bo dùng hết sạch GPIO nên IR phải mượn chân của module 4G không dùng tới, và không còn chân cho DHT (xem hàng dưới). STM32 cần thêm module WiFi rời (vd. ESP-01) → phức tạp hoá BOM và firmware một cách không cần thiết khi ESP32 đã tích hợp sẵn WiFi + BLE. |
| MCU node outdoor | **ESP32 DevKit V1** | ESP8266, ESP32-C3, module cảm biến Zigbee rời | Tác vụ thì nhẹ (chỉ đọc DHT + gửi định kỳ) và **ESP8266 rẻ hơn**, nhưng **đã chuyển sang ESP32 để hai node dùng chung một dòng chip**: ESP-NOW trên ESP8266 dùng `espnow.h` kiểu SDK cũ (có `role`, `add_peer` truyền rời từng tham số), trên ESP32 dùng `esp_now.h` của IDF (`esp_now_peer_info_t`, không có role) — hai bộ API khác hẳn nhau cho *cùng một giao thức*. Giữ hai chip nghĩa là mỗi lần sửa giao thức ESP-NOW phải sửa hai lần theo hai kiểu, đúng loại việc dễ quên một nửa. Chênh lệch giá vài chục nghìn/node không bù nổi rủi ro đó. Zigbee cần thêm 1 coordinator riêng biệt — tăng độ phức tạp hạ tầng không cần thiết cho quy mô 1 outdoor/nhà. |
| Cảm biến nhiệt độ/độ ẩm | **SHT3x (I2C) ở node indoor · DHT22 ở node outdoor** | DHT ở cả hai, BME280/BME680, AHT20 | Node outdoor giữ DHT22: rẻ, dễ mua tại VN, ±0.5 °C là đủ cho running-mean chứ không cần độ chính xác phòng lab. Node indoor **buộc phải** đổi sang SHT3x — không phải để nâng cấp cho vui mà vì bo QR Box Advance đã dùng GPIO4 làm I2C SCL và không còn chân trống nào cho một dây DHT; bus I2C thì đã sẵn trở kéo và đang chạy cho DS1307. Chọn **0x44** vì AHT20 nằm ở 0x38, **trùng chip cảm ứng FT6236** trên cùng bus. Phần thưởng kèm theo: SHT3x chính xác hơn DHT22 đáng kể, mà đây lại đúng là số đo đi vào thuật toán comfort. |
...
| Ingress / TLS | **Cloudflare Tunnel** (HTTP/HTTPS) | Nginx reverse proxy + Let's Encrypt (mở port trực tiếp), VPN (WireGuard) | Cloudflare Tunnel phục vụ **web + app** mà không cần mở port HTTP nào, tự động TLS, miễn phí, dễ setup hơn tự quản lý renew chứng chỉ Let's Encrypt. **Đính chính 21/07/2026:** câu "0 cổng public ngoài SSH" **không còn đúng** — CF Tunnel chỉ chở được HTTP(S), không chở MQTT thô, nên **cổng 1883 đã phải mở trực tiếp ra internet** cho các node ESP kết nối. Đã bật **authentication per-device** và **topic ACL** trên EMQX (21/07/2026), cả hai **đọc thẳng bảng `devices` trong Postgres** (nguồn `postgresql` của EMQX). Nhờ vậy tạo node trên web là dùng được ngay và xoá node là mất quyền ngay — **không còn bước nạp credential/ACL thủ công vào EMQX** để quên. Luật: mỗi node chỉ được publish/subscribe trong `bl/{org của chính nó}/#`, worker được `bl/#`. Khoá ở mức **org chứ không mức uuid** là cố ý — node master phải publish HỘ các slave lên topic riêng của slave (relay ESP-NOW), khoá cứng theo uuid sẽ chặn nhầm chính luồng đó.

**Hai bẫy đã vấp, ghi lại để khỏi mất thời gian lần sau:**
1. `acl.conf` mặc định kết thúc bằng `{allow, all}.` → nguồn `file` cho qua mọi thứ mà nguồn khác không khớp, nên `no_match=deny` không bao giờ tới lượt và ACL chỉ là trang trí. Phải **tắt nguồn `file`**.
2. Kiểm chứng bằng client MQTT phải đọc **mã CONNACK trong `on_connect(rc)`**, không được chỉ xem `connect()` có ném lỗi hay không: broker từ chối vẫn cho mở TCP rồi mới gửi `rc=4/5` và đóng. Phép đo sai kiểu này báo "ai cũng vào được" cho mọi trường hợp và suýt dẫn tới kết luận hệ thống mất an toàn.

Đã kiểm chứng bằng dụng cụ đúng: credential lạ → `rc=5`, sai mật khẩu → `rc=4`, đúng credential → `rc=0`; và publish sang org khác bị chặn. Mở port + Nginx tự quản lý sẽ phải tự vá lỗ hổng và theo dõi cert hết hạn. VPN phù hợp cho truy cập nội bộ nhưng không tiện cho việc phục vụ traffic công khai từ khách hàng (app/web cần dùng được mà không phải cài VPN client). |
...
|---|---|---|---|---|
| 1 | ~~**Node indoor (còn thiếu)**~~ → **ĐÃ LÀM** | `FirmWare/esp32-indoor/` — bo QR Box Advance + `IRremoteESP8266`, phủ đủ 5 topic `telemetry`/`status`/`cmd`/`state`/`learn`, có học remote + cache mã IR trong NVS, thêm màn cảm ứng tại chỗ | Đúng như đề xuất là dùng `IRremoteESP8266` thay vì tự viết. **Khác đề xuất ở một điểm:** node nối WiFi/MQTT **trực tiếp**, KHÔNG làm ESP-NOW slave của master — lệnh `ir_raw` cỡ vài KB không lọt qua giới hạn 250 byte/gói của ESP-NOW (xem mục 2) | `FirmWare/esp32-indoor/` |
| 2 | ~~**Hoàn thiện ESP-NOW**~~ → **ĐÃ CHỐT KHÁC** | Giữ ESP-NOW **chỉ cho chiều outdoor → indoor** (gói 43 byte). Node indoor không làm slave mà kiêm luôn vai trò master, nối MQTT thẳng | Đề xuất ban đầu (chuyển *cả* indoor sang ESP-NOW slave) không khả thi: chiều cloud → indoor chở `ir_raw` vài KB, muốn qua ESP-NOW phải tự viết chia mảnh + ghép lại + báo thiếu mảnh — trong khi MQTT đã lo sẵn. ESP-NOW vẫn đúng cho outdoor vì gói chỉ 43 byte và một chiều | `esp32-indoor`, `esp32-outdoor` |
| 2b | ~~**Dọn node ESP32-S3**~~ → **ĐÃ DỌN THẬT** (27/07/2026) | Chốt **một bo cho một vai trò**: indoor = QR Box Advance (env `qrbox-touch` duy nhất), outdoor = ESP32 DevKit V1. Đã xoá hẳn `esp32s3-indoor-master/` (bản thử nghiệm không IR) và `esp8266-outdoor/`; `esp32-indoor/platformio.ini` từ 3 env rút còn 1 | Bản trước giữ 3 build environment dùng chung mã nguồn để "khỏi chép code". Nhưng hai trong ba env trỏ tới bo **không có trong tay** — cấu hình không ai build và không ai kiểm chứng được, lại kéo theo code chết (nhánh đọc DHT, dò tên bo lúc chạy) và một `lib_deps` DHT không còn ai dùng. Rủi ro "chép code trôi xa nhau" nay xử lý bằng cách khác: `espnow-relay.*`/`slave-watch.*` chỉ còn **một bản** ở node indoor, khuôn gói tin dùng chung qua `shared/espnow-message.h` | `FirmWare/esp32-indoor/platformio.ini` |
